...
const nativeValue = computed({
  ...
  set: (val: string) => {
    ...
    if (isValid(parsed)) {
      ...
      selectedTime.value = timePart || '12:00'
      const localDateTime = new Date(`${datePart}T${timePart || '12:00'}`)
      emit('update:modelValue', localDateTime.toISOString())
    }
  }
})
</script>
...
<template>
  <!-- iOS: native datetime-local input -->
